...
from core.models import Skill, Achievement, AdminMessage, UserProfile, UserSkill

class Command(BaseCommand):
    help = 'Populate database with initial skills and sample data'

    def handle(self, *args, **options):
        self.stdout.write('🚀 Populating database with initial data (skills, achievements, admin messages)...')
        ...
        skills_data = [
            ...
            ('Machine Learning', 'Programming', 'AI and machine learning algorithms'),
        ...
        for name, category, description in skills_data:
            ...
            if created:
                ...
                self.stdout.write(f"⚠️  Skill already exists: {name}") # Added warning for existing skills
        ...
        for name, description, icon, points in achievements_data:
            ...
            if created:
                ...
                self.stdout.write(f"⚠️  Achievement already exists: {name}") # Added warning for existing achievements
        ...
        messages_data = [
            ...
            ('New Skills Added', 'We have added 40+ new skills across different categories. Check out the skills management page to add them to your profile!'),
        ...
        for title, content in messages_data:
            ...
            if created:
                created_messages += 1
                self.stdout.write(f"📢 Created admin message: {title}")
            else:
                self.stdout.write(f"⚠️  Admin message already exists: {title}") # Added warning for existing messages
        
        # Removed sample user creation and their skill assignments
        self.stdout.write(
            self.style.SUCCESS(
                f'\n✅ Database population complete!\n'
                f'   - {created_skills} new skills created\n'
                f'   - {created_achievements} new achievements created\n'
                f'   - {created_messages} new admin messages created\n'
                f'\n📊 Total in database:\n'
                f'   - Skills: {Skill.objects.count()}\n'
                f'   - Achievements: {Achievement.objects.count()}\n'
                f'   - Admin Messages: {AdminMessage.objects.count()}\n'
                f'   - Users: {User.objects.count()} (Only users registered via the app or createsuperuser)\n'
            )
        )
